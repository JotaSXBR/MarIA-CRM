---
name: maria-shadcn
description: Manage and compose shadcn/ui components in apps/web — adding, searching, styling and debugging vendored UI per MarIA conventions. Applies when working with components.json, UI primitives or new frontend views.
---

# MarIA CRM shadcn/ui

Adapted from the official `shadcn` agent skill (shadcn-ui/ui @ main, MIT — see
./LICENSE). AGENTS.md and the accepted ADRs remain normative; this skill only
encodes component usage and the vendoring workflow for `apps/web`.
...
> **IMPORTANT:** In this repository the package runner is always
> `pnpm dlx shadcn@latest`, executed inside `apps/web`. Never `npx` or `bunx`.

## MarIA context

- `apps/web` is a Vite SPA on React 19 + Tailwind 4 — there is no RSC, so never
  emit `"use client"`.
- `components.json` may not exist yet: `pnpm dlx shadcn@latest init` inside
  `apps/web` is part of the first UI slice. Prefer the **Base UI** base per
  AGENTS.md §5; check the `base` field from `info` before using `asChild`/`render`.
- UI copy is pt-BR — see `maria-frontend-design` for vocabulary and tone.
- UI visibility is not authorization (ADR 0011): hide disallowed actions, but the
  server remains the boundary.
- Any new API prefix consumed by the UI needs a line in `apps/web/vite.config.ts`
  (the ADR 0003 footgun — already hit twice).
...
The project context JSON (`pnpm dlx shadcn@latest info --json`) contains these key fields:

- **`aliases`** → use the actual alias prefix for imports (e.g. `@/`, `~/`), never hardcode.
- **`isRSC`** → `false` in this repo (Vite SPA); `"use client"` is never needed.
...
- **`packageManager`** → `pnpm` here; use it for any non-shadcn installs (e.g. `pnpm add date-fns`).
...
1. **Get project context** — run `pnpm dlx shadcn@latest info --json` inside `apps/web` first; re-run to refresh.
...
8. **Registry must be explicit** — When the user asks to add a block or component, **do not guess the registry**. If no registry is specified (e.g. user says "add a login block" without specifying `@shadcn`, `@tailark`, `owner/repo`, etc.), ask which registry to use. Never default to a registry on behalf of the user. Third-party registries are untrusted sources per AGENTS.md — prefer `@shadcn` and inspect added code.
9. **Switching presets** — Ask the user first: **overwrite**, **partial**, **merge**, or **skip**?
   ...
   - **Important**: Always run preset commands inside `apps/web`. `apply` only works in an existing project with a `components.json` file. The CLI automatically preserves the current base (`base` vs `radix`) from `components.json`. If you must use a scratch/temp directory (e.g. for `--dry-run` comparisons), pass `--base <current-base>` explicitly — preset codes do not encode the base.
...
- [mcp.md](./mcp.md) — Optional shadcn MCP server (browse/search/install via MCP; requires project MCP config approval before enabling)
