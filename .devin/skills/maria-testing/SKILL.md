...
test("tenant isolation enforced", async () => {
  ...
  await admin.query(
    "INSERT INTO contacts (workspace_id, name) VALUES ($1, 'A')",
    [workspaceA],
  );
  await admin.query(
    "INSERT INTO contacts (workspace_id, name) VALUES ($1, 'B')",
    [workspaceB],
  );
...
test("context does not leak on pooled connection", async () => {
  await database.withWorkspace(workspaceA, async (tx) => {
    await tx.execute(
      sql`INSERT INTO contacts (workspace_id, name) VALUES ($1, 'Test')`,
      [workspaceA],
    );
